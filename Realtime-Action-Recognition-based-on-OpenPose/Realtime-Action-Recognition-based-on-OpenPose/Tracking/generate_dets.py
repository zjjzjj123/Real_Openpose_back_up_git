...
class ImageEncoder(object):
    def __init__(self, checkpoint_filename, input_name="images", output_name="features"):
        ...
        assert len(self.output_var.get_shape()) == 2 #T.get_shape()不需要放在sess运行即可得到shape
        assert len(self.input_var.get_shape()) == 4   #T.get-shape返回的元组
        self.feature_dim = self.output_var.get_shape().as_list()[-1] #作为list返回
...
def create_box_encoder(model_filename, input_name="images", output_name="features", batch_size=32):
    ...
    def encoder(image, boxes): #源图片 图片中检测到的物体坐标
        image_patches = []
        for box in boxes:
            patch = extract_image_patch(image, box, image_shape[:2]) #box 没归一化的 [128,64]
            print('box:',box)
        ...
        return image_encoder(image_patches, batch_size) #调用__call__ 计算特征
